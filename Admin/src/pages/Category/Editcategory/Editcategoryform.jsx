...
const Editcategoryform = ({ id }) => {
  ...
  initialValues={{
    ...
    parent_category: data.data.parentcategory.map((cat) => cat._id || cat), // Ensure only ObjectIds
  ...
  onSubmit={(values) => {
    ...
    // ✅ Ensure `parent_category` is an array and append each ID separately
    if (Array.isArray(values.parent_category)) {
      values.parent_category.forEach((catId) => {
        formdata.append("parent_category[]", catId); // Append each ObjectId separately
      });
    }
