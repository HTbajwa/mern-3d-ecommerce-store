...
useEffect(()=>{
  ...
  <h4>CATEGORIES</h4>
  {data4 && data4.parentcategoryname ? (
    <>
      <h6
        style={{
          color: 'black',
    ...
    <h6 style={{ color: '#878787', padding: `10px 0px 10px 8px` }}>
      {data4?.categorydata?.name || 'No category available'}
    </h6>
  )}
</div>
  ...
  <h3
    className={brand ? "collapse-block-title dynamic-after2" : "collapse-block-title dynamic-after"}
    onClick={() => setbrand(prev => !prev)}
  >
    brand
  </h3>
  <div className="collection-collapse-block-content" style={{ display: brand ? "block" : "none" }}>
    <div className="collection-brand-filter">
      {data4.availableFilters.brand.map((item, index) => (
        <div key={`${item}-${index}`} className="custom-control custom-checkbox form-check collection-filter-checkbox">
          <input
            type="checkbox"
            ...
            id={`item-${index}-${item}`}
            checked={url.includes(`brand=${item}`)}
          />
          <label className="custom-control-label form-check-label" htmlFor={`item-${index}-${item}`}>
            {item}
          </label>
        </div>
      ))}
